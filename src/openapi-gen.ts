import { GenerateOptions, Repository } from "./type";
import { basename, extname, sep, resolve } from "node:path";
import { shell, isExistBranch, getProtoFileInDirectory } from "./helper"
import {negate, checkCommandIsExist, loadConfiguration} from "./helper"
...
class Generate {
  ...
  constructor(opt: GenerateOptions) {
    ...
    const { debug, buildDir, gitRepository, outputDir, disabledRemoveCommand } = opt as Required<GenerateOptions>
    this.debug = debug;
  ...
  private checkOption( opts: GenerateOptions ) {
    ...
    if (!opts.outputDir) throw new Error( "Requires `outputDir` configuration." );
  }
...
// 从配置文件中获取配置
async function withConfiguration ( args: GenerateOptions | undefined ):Promise<GenerateOptions> {
...
export async function generate(opts?: GenerateOptions ) {
  ...
  const conf = await withConfiguration(opts)
