...
class Generate {
  ...
  constructor(opt) {
    ...
    const { debug, buildDir, gitRepository, openapiDir, disabledRemoveCommand } = opt;
    this.debug = debug;
  ...
  async pullGitRepo() {
    ...
    for (const repo of this.gitRepository) {
      ...
      await promises$1.setTimeout(1000);
    }
  }
  ...
  async genOpenApi() {
    ...
    for (const repo of this.gitRepository) {
      ...
      const paths = await getProtoFileInDirectory(node_path.resolve(rootDir, "proto", serviceName(repo.url)));
