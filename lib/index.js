...
class Generate {
  ...
  async cloneRepo(repo, branch = "master", path = "") {
    ...
    await this.runShell(`cd ${this.buildDir}/proto && git clone --depth 1 ${repoUrl}`).catch((err) =>
    ...
    if (path && path !== ".") {
      const sourcePath = node_path.resolve(process.cwd(), `./${this.buildDir}/proto/${service}/${path}`);
      // 目录不存在就不管他。
  ...
  async generate() {
    ...
    // 如果目录已存在就不重新拉 proto
    if (await negate(checkPathIsExist(node_path.resolve(process.cwd(), `./${this.buildDir}/proto`)))) {
