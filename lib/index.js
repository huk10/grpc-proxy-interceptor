...
var openapiClientAxios = require("openapi-client-axios");
...
async function isExistService(proto, options = {}) {
  const packageDefinition = await protoLoader__namespace.load(proto, options);
...
class Generate {
  ...
  constructor(opt) {
    ...
    const { debug, buildDir, openapiDir, accessToken, gitRepository, disabledRemoveCommand, extraProtoIncludeDirs } =
      opt;
    this.debug = debug;
  ...
  checkOption(opts) {
    ...
    if (!opts.openapiDir) throw new Error("Requires `openapiDir` configuration.");
  }
  // 运行一个 shell 命令
  runShell(command) {
  ...
  // 将 git repo 的指定目录拉下来
  async archiveRepo(repo, branch = "master", path = "") {
  ...
  async checkoutBranch(service, branch) {
    try {
      await this.runShell(`cd ${this.buildDir}/${service} && git checkout ${branch}`);
      if (this.debug) {
  ...
  async cloneRepo(repo, branch = "master", path = "") {
    ...
    path = pathNormalize(path);
    await this.runShell(
      `cd ${this.buildDir} && git clone --depth 1 ${protocolConvert(repo, this.accessToken)} ${service}`
    );
    if (branch !== "master") {
    ...
    if (path && path !== ".") {
      if (
        await negate(checkPathIsExist(node_path.resolve(process.cwd(), `./${this.buildDir}/proto/${service}${path}`)))
      ) {
        if (!this.disabledRemoveCommand) {
          await this.runShell(`rm -rf ./${this.buildDir}/proto/${service}`);
        }
        return;
      }
      await this.runShell(`mv ./${this.buildDir}/proto/${service} ./${this.buildDir}/.stash/${service}`);
      await this.runShell(`mv ./${this.buildDir}/.stash/${service}/${path} ./${this.buildDir}/proto/${service}`);
    }
  }
  // 将一组 git repo 拉下来
  async pullGitRepo() {
    await this.runShell(
      `rm -rf ./${this.buildDir} && mkdir -p ./${this.buildDir}/.stash/ && mkdir -p ./${this.buildDir}/proto/`
    );
    ...
    // ...又是下划线又是中划线。。。
    if (await checkPathIsExist(node_path.resolve(process.cwd(), `./${this.buildDir}/proto/y_common`))) {
      await this.runShell(`cp -r ./${this.buildDir}/proto/y_common ./${this.buildDir}/proto/y-common`);
    }
  }
  // 生成 openapi 的 json 文件
  async genOpenApi() {
    // 一个服务一个服务的处理
    await this.runShell(`rm -rf ./${this.openapiDir} && mkdir ${this.openapiDir}`);
    ...
    for (const repo of this.gitRepository) {
      const mParams = [];
      const rootDir = node_path.resolve(process.cwd(), this.buildDir);
      const includeDirs = [this.buildDir, this.buildDir + "/proto/", ...this.extraProtoIncludeDirs];
      const paths = await getProtoFileInDirectory(
        node_path.resolve(rootDir, "proto", serviceName(repo.url)),
        includeDirs
      );
      if (paths.length === 0) {
        continue;
      }
      const proto = paths.map((value) => value.replace(rootDir + "/proto/", ""));
      for (const u of proto) {
        mParams.push(`--openapiv2_opt=M${u}=example.com/${u.replace(node_path.extname(u), "")}`);
      }
      await this.runShell(
        [
          `protoc ${includeDirs.map((value) => `--proto_path=${value}`).join(" ")}`,
...
function firstPart(path) {
  return pathNormalize(path).split(node_path.sep)[0];
}
...
function serviceName(repo) {
  return node_path.basename(repo, node_path.extname(repo)).replace(/-/g, "_");
}
// 针对性的处理。
function protocolConvert(repo, accessToken) {
  if (/^git@/.test(repo)) {
    return `https://oauth2:${accessToken}@${repo.replace(/^git@/, "").replace(":", "/")}`;
  }
  if (/^https?:\/\//.test(repo)) {
    if (/^https/.test(repo)) {
...
function httpStatus2GrpcStatus(code) {
  switch (code) {
    ...
    case httpStatusCodes.StatusCodes.TOO_MANY_REQUESTS:
      return grpcJs.status.RESOURCE_EXHAUSTED;
    case httpStatusCodes.StatusCodes.BAD_REQUEST:
      return grpcJs.status.INVALID_ARGUMENT;
    case httpStatusCodes.StatusCodes.CONFLICT:
      return grpcJs.status.ABORTED;
    case httpStatusCodes.StatusCodes.NOT_IMPLEMENTED:
      return grpcJs.status.UNIMPLEMENTED;
    case httpStatusCodes.StatusCodes.SERVICE_UNAVAILABLE:
...
class ApiProxy {
  ...
  async call(callPath, metadata, body) {
    try {
      ...
      const { operationId, document, method, parameters, filePath } = definition;
      const params = handleParameters(parameters, body);
      const client = await new openapiClientAxios.OpenAPIClientAxios({
        definition: document,
      }).init();
      const result = await client[operationId](params, method === "GET" ? null : body, {
        baseURL: typeof this.getaway === "function" ? this.getaway({ callPath, filePath }) : this.getaway,
        headers: toMetadataHeader(metadata),
        params: method === "GET" ? qs__namespace.stringify(body) : undefined,
      });
...
function openapiInterceptorSync(opts) {
  const apiProxyRef = { apiProxy: null };
  ...
  return function interceptorImpl(options, nextCall) {
    ...
    return new grpcJs.InterceptingCall(nextCall(options), {
      ...
      halfClose: async function (next) {
        ...
        // 此刻的 value 类型是 [MedataValue]
        // call 方法保证即使是内部错误，也会返回一个正确的结构
        const result = await apiProxyRef.apiProxy.call(options.method_definition.path, metadata, message);
        listener.onReceiveMessage(result.response);
        listener.onReceiveMetadata(result.metadata);
        listener.onReceiveStatus(result.status);
      },
  ...
  return function interceptorImpl(options, nextCall) {
    ...
    return new grpcJs.InterceptingCall(nextCall(options), {
      ...
      halfClose: async function (next) {
        ...
        // call 方法保证即使是内部错误，也会返回一个正确的结构
        const { metadata, message, listener } = ref;
        const baseUrl = typeof getaway === "function" ? getaway(options.method_definition.path) : getaway;
        const result = await proxyTo(baseUrl + options.method_definition.path, message, metadata);
        listener.onReceiveMessage(result.response);
        listener.onReceiveMetadata(result.metadata);
        listener.onReceiveStatus(result.status);
      },
