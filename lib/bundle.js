...
var SwaggerClient__namespace = /*#__PURE__*/_interopNamespace(SwaggerClient);
...
async function loadConfiguration(path = "./openapi-proxy.config.js") {
    const configurationPath = node_path.resolve(process.cwd(), path);
    if (await (checkPathIsExist(configurationPath))) {
        const result = await (function (t) { return Promise.resolve().then(function () { return /*#__PURE__*/_interopNamespace(require(t)); }); })(configurationPath);
...
class ApiProxy {
    ...
    async call(callPath, metadata, body) {
        ...
        const request = SwaggerClient__namespace.buildRequest({
            spec: spec,
            operationId,
            parameters: body,
        ...
        const result = await SwaggerClient__namespace.http(request).catch((err) => err.response);
        const responseMetadata = toMetadata(getResponseMetadata(result.headers));
        const code = result.ok ? grpcJs.status.OK : result.body.code;
        return {
...
async function parseOpenApiSpec(url) {
    const str = await promises$1.readFile(url, "utf8");
...
exports.grpcGatewayProxyInterceptor = grpcGatewayProxyInterceptor;
