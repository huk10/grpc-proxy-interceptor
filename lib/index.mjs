...
class Generate {
  ...
  async cloneRepo(repo, branch = "master", path = "") {
    ...
    path = pathNormalize(path);
    await this.runShell(
      `cd ${this.buildDir} && git clone --depth 1 ${protocolConvert(
        repo,
        this.accessToken
      )} ${service}`
    );
    ...
    if (path && path !== ".") {
      if (
        await negate(
          checkPathIsExist(
            resolve(process.cwd(), `./${this.buildDir}/proto/${service}${path}`)
          )
        )
      ) {
        if (!this.disabledRemoveCommand) {
...
export { generate, interceptor, openapiInterceptor, openapiInterceptorSync };
