...
describe("helper: Protobuf functions", () => {
  ...
  test("isExistService:", async () => {
    ...
    expect(
      await isExistService(
        resolve(dirname, "../resources/proto/common.proto"),
        options
      )
    ).toBe(false);
  });
  ...
  test("getProtoFileInDirectory:no-includeDirs-params", async () => {
    expect(
      await getProtoFileInDirectory(resolve(dirname, "../resources/proto"))
    ).toEqual([resolve(dirname, "../resources/proto/greeter.proto")]);
  });
  test("getProtoFileInDirectory:exist-includeDirs-params", async () => {
    expect(
      await getProtoFileInDirectory(
        resolve(dirname, "../resources/proto"),
        options.includeDirs
      )
    ).toEqual([resolve(dirname, "../resources/proto/greeter.proto")]);
  });
