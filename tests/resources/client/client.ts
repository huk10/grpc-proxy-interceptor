...
import {interceptor as openapiInterceptor} from '../../../src/openapi-interceptor'
...
const grpGatewayServerDefinition = resolve(dirname, '../grpc-server/proto/greeter/v1/services/greeter.proto')
// 服务端提供 grpc-web+json 的实现（grpc-gateway 也有实现这个。）
const grpGatewayServerDefinitionV2 = resolve(dirname, '../grpc-server/proto/greeter/v2/services/greeter.proto')

const packageDefinition = protoLoader.loadSync(grpGatewayServerDefinition, options)
const packageDefinitionV2 = protoLoader.loadSync(grpGatewayServerDefinitionV2, options)

const grpcObject = grpc.loadPackageDefinition(packageDefinition)
const grpcObjectV2 = grpc.loadPackageDefinition(packageDefinitionV2)

// @ts-ignore
const GreeterClient = grpcObject.example.greeter.v1.services.Greeter as typeof grpc.Client

// @ts-ignore
const GreeterClientV2 = grpcObjectV2.example.greeter.v2.services.Greeter as typeof grpc.Client
...
export async function clientWithGrpcWeb() {
  return new GreeterClientV2('127.0.0.1:9091', grpc.credentials.createInsecure(), {
    interceptors: [
      interceptor({
        // grpc-getaway 服务地址
        getaway: 'http://127.0.0.1:4501',
