...
export interface GreeterClient extends ServiceClient {
  SayHello(
    body: { name: string },
    md: Metadata,
    callback: UnaryCallback<{ message: string }>
  ): ClientUnaryCall;
  SayHello(
    body: { name: string },
    callback: UnaryCallback<{ message: string }>
  ...
  Status(
    body: { status: Status; errorMsg?: string },
    md: Metadata,
    callback: UnaryCallback<void>
  ): ClientUnaryCall;
  Status(
...
export function testGrpcRequest(getClient: () => GreeterClient) {
  ...
  test(`Status`, async () => {
    const client = await getClient();
    async function callWithStatus(
      status: Status,
      msg?: string
    ): Promise<StatusObject> {
      return new Promise((resolve1, reject) => {
        const emitter = client.Status(
          { status: status, errorMsg: msg },
          () => void 0
        );
